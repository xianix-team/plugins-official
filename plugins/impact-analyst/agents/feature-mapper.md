...
The orchestrator passes you the changed file list, patches, and a **pre-fetched codebase fingerprint** that includes files already known to import/reference the changed modules. Use these as your primary source — do not re-run `git diff`.
...
**Tool call budget:** Aim for no more than **10–15 Grep/Glob calls** and **5–8 Read calls** total. Focus on changed files and their direct consumers. If you must stop early, mark areas as "not fully traced" rather than skipping silently.
